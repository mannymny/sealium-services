import shutil, zipfile, os, traceback, tempfile
...
class CreateFileProofUseCase:
    def __init__(self, forensics, tsa, monitor, output_root, keep_dir: bool = True):
    ...
    async def execute(self, path: str, forensic_mode: bool) -> dict:
        try:
            ...
            case_name = f"{ts_str}_{sanitize_filename(target.stem)}"
            temp_dir = None
            if self.keep_dir:
