import shutil, zipfile, os, traceback, socket, ssl, re, tempfile
...
class CreateUrlProofUseCase:
    def __init__(self, capture, tsa, monitor, output_root, keep_dir: bool = True):
    ...
    async def execute(self, url: str) -> dict:
        try:
            ...
            if m:
                user = sanitize_filename(m.group(1))
                tweet_id = sanitize_filename(m.group(2))
            if user and tweet_id:
                case_name = f"{ts_str}_{user}_{tweet_id}"
            else:
                host_safe = sanitize_filename(url.split("//")[-1].split("/")[0])
                case_name = f"{ts_str}_{host_safe}"
            temp_dir = None
            if self.keep_dir:
            ...
            with open(case_dir / "manifest.json", "w") as f: f.write(case.model_dump_json(indent=2))
            ...
            result = {"status": "success", "case_id": case.id, "zip_path": str(zip_path)}
            if self.keep_dir:
                result["path"] = str(case_dir)
            return result
